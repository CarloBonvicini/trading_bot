...
- **`ricerca.py`** — cercare con un **budget** invece di provare tutto. `esplora(griglia, valuta, budget=...)` semina un reticolo largo su tutto lo spazio e poi spende il resto del budget sui **vicini dei migliori**. Regola che tiene fermo il passato: se la griglia intera sta nel budget viene enumerata, quindi ogni ricerca che già costava poco dà esattamente le stesse risposte. Lo spazio si misura con `quante_combinazioni()` **senza costruirlo** — su un milione di punti materializzarli per provarne cinquecento sarebbe il problema, non la soluzione. Il meccanismo è misurato, non ereditato dal piano: tirare a caso **non** paga su questo catalogo (dieci strategie su diciotto hanno due parametri), mentre affinare intorno ai migliori raggiunge l'ottimo della griglia fitta in 4 casi su 6 allo stesso costo di oggi.
- **`walkforward.py`** — finestre IS/OOS scorrevoli. La ricerca guidata valuta una combinazione su **tutte le finestre in un colpo solo** (il segnale nasce una volta sull'intera serie: è il warm-up, non si tocca), e ogni finestra continua a scegliersi la sua migliore fra quelle provate. Il budget decide *quali* combinazioni guardare, non chi vince in ciascuna finestra; il prezzo dichiarato è che una combinazione ottima in una finestra sola e pessima altrove ha meno probabilità di essere esplorata. Il segnale di ogni combinazione si calcola **una volta sull'intera serie** e poi si ritaglia sulla finestra: è il warm-up degli indicatori (senza, un indicatore più lungo della finestra resta piatto a zero) ed evita di ricalcolare lo stesso segnale per ogni finestra. A parità di punteggio vince sempre una combinazione che ha operato: quelle inerti hanno Sharpe 0 secco e batterebbero tutte quelle in perdita.
...
- **Mai costruire un paesaggio di test con `hash()`**: Python lo randomizza a
  ogni processo, quindi il test passa da solo e fallisce dentro la suite (o,
  peggio, fallisce in CI una volta ogni tanto senza motivo apparente). L'indice
  si ricava dalle coordinate.
